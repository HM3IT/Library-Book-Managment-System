...
public class MemberMainMenu extends JFrame {

	/**
	 * Generated serial version ID
	 */
	private static final long serialVersionUID = 4155062920312677951L;
...
// button name
	...
	private char[] userPass = null;
	private JTextField txtMemberName;
	private JTextField txtMemberPass;

	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					MemberMainMenu frame = new MemberMainMenu();
...
// Menu button action event		
		...
		panelHome.addMouseListener(new MouseAdapter() {
			...
			public void mousePressed(MouseEvent e) {
				...
				invisibleAllTab();
				homeTab.repaint(); // to update the new appearance of the component on the GUI but have not made
									// any changes to its size
				homeTab.setVisible(true);
			}
		});
		...
		panelSingOut.addMouseListener(new MouseAdapter() {
			@Override
			public void mousePressed(MouseEvent e) {
				dispose();// closing the main form JFrame
...
//JFrame dragged moving implementation
	...
	public JPanel createSettingTab(JPanel panelMain) {
		...
		JLabel lblAccountInformation = new JLabel(username + " account information");
		lblAccountInformation.setForeground(Color.BLUE);
		lblAccountInformation.setHorizontalAlignment(SwingConstants.CENTER);
		lblAccountInformation.setFont(new Font("Cambria Math", Font.PLAIN, 23));
		lblAccountInformation.setBounds(337, 30, 309, 54);
		panelSettingTab.add(lblAccountInformation);
		...
		txtMemberName.setFont(new Font("Arial", Font.PLAIN, 16));
		txtMemberName.setBounds(479, 161, 142, 22);
		panelSettingTab.add(txtMemberName);
		txtMemberName.setColumns(10);

		txtMemberPass = new JTextField();
		txtMemberPass.setFont(new Font("Arial", Font.PLAIN, 16));
		txtMemberPass.setColumns(10);
		txtMemberPass.setBounds(479, 223, 142, 22);
		panelSettingTab.add(txtMemberPass);

		System.out.println(username);
		String user = username;
		char[] userPassword = userPass == null ? "Need to login".toCharArray() : userPass;
		String password = new String(userPassword);
		txtMemberName.setText(user);
		txtMemberPass.setText(password);
		return panelSettingTab;
	}

	// return JPanel homeTab with visible property false
	public JPanel createHomeTab(Image bookManageImg, Image memberManageImg, JPanel panelMain) {
		...
		JLabel lblTitle = new JLabel(username + " Welcome Back");
		...
		JLabel lblBookSearch = new JLabel("Book Management");
		lblBookSearch.setFont(new Font("Arial", Font.PLAIN, 15));
		lblBookSearch.setHorizontalAlignment(SwingConstants.CENTER);
		lblBookSearch.setBounds(30, 242, 150, 27);
		panelHomeTab.add(lblBookSearch);
